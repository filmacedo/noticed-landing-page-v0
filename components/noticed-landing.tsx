  ...
  const renderContent = (block: ContentBlock, index: number) => {
    ...
    if (block.type === "brand-inline") {
      return (
        <p 
          key={index} 
          className="mb-8 text-2xl md:text-3xl font-display tracking-[-0.04em]"
  ...
  return (
    <div className="min-h-screen font-[var(--font-body-editorial)] text-[#f0ede8] lowercase">
      ...
      >
        ...
        {/* Manifesto stanzas - Newsreader for editorial */}
        <div className="max-w-xl text-lg md:text-xl leading-relaxed font-normal">
          {content.map((block, index) => renderContent(block, index))}
        </div>

        {/* CTA - Instrument Sans (functional), solid brand button */}
        <a
          href="#"
          className="inline-block mt-12 px-6 py-2.5 font-[var(--font-body-functional)] text-[13px] font-semibold normal-case tracking-normal no-underline rounded-md bg-[#f0ede8] text-[#0a0a0a] transition-opacity duration-300 hover:opacity-90"
